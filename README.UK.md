...
- **Прикладний ярус:** `app`, `observe`, `jwt`, `auth`, `argon2id`, `session`, `mail`, `cli`.
...
| **Ідентичність** | `auth`, `argon2id`, `jwt`, `session` | Хешують паролі, видають і перевіряють токени, захищають маршрути, тримають браузерні сесії. |
...
- [**argon2id** - memory-hard хешування паролів Argon2id](#argon2id)
...
Той самий інструмент застосовує міграції - і перевіряє, перш ніж запускати:
файл, що комітить сам, відхиляється, застосований файл, який редагували чи
видалили, зупиняє запуск, no-transaction файл, що впав посередині, ніколи не
повторюється мовчки, а паралельні запуски стають у чергу на лок.
`pgc migrate status` і `pgc check` падають у CI, коли історія міграцій чи
згенерований код розійшлися. pgc стабільний з v1: його команди, конфігурація,
lock-файл, таблиця історії й згенерований код не змінюються несумісно в межах
v1.
...
`argon2id` хешує й перевіряє паролі за Argon2id (RFC 9106) - memory-hard
функцією, рекомендованою для зберігання паролів. Memory-hard означає, що кожен
хеш коштує фіксований обсяг RAM, тож викрадений хеш дорого зламувати офлайн. Він
реалізує Argon2id і його внутрішній BLAKE2b на самій стандартній бібліотеці, без
залежностей, і закріплений за офіційними тест-векторами RFC.

Кодований хеш - самоописовий PHC-рядок, а набір методів `Hasher` збігається з
інтерфейсом `PasswordHasher` у `auth`, тож підставляється туди без взаємного
імпорту пакетів.
...
func main() {
	...
	encoded, _ := h.Hash([]byte("correct horse battery staple")) // зберігаєте
	_ = h.Verify(encoded, []byte("correct horse battery staple")) // nil при збігу
}
```

**Детальніше:** [github.com/goloop/argon2id](https://github.com/goloop/argon2id) · [довідник](https://pkg.go.dev/github.com/goloop/argon2id)
